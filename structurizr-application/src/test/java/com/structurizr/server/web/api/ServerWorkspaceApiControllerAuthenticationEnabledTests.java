...
public class ServerWorkspaceApiControllerAuthenticationEnabledTests extends AbstractTestsBase {
    ...
    @Test
    void regenerateApiKey_ReturnsAnError_WhenTheWorkspaceDoesNotExist() {
        controller.setWorkspaceComponent(new MockWorkspaceComponent() {
            @Override
            public WorkspaceMetadata getWorkspaceMetadata(long workspaceId) {
                return null;
    ...
    @Test
    void regenerateApiKey_ReturnsAnError_WhenNoApiKeyIsSpecified() {
        try {
            controller.regenerateApiKey(1, "");
            fail();
        } catch (HttpUnauthorizedException e) {
    ...
    @Test
    void regenerateApiKey_ReturnsAnError_WhenTheApiKeyIsIncorrect() {
        try {
            controller.regenerateApiKey(1, "0987654321");
            fail();
        } catch (HttpUnauthorizedException e) {
            assertEquals("Incorrect API key", e.getMessage());
        }
    ...
    @Test
    void regenerateApiKey_ReturnsAnError_WhenTheApiKeyIsCorrectAndTheWorkspaceIsLocked() {
        try {
            workspaceMetadata.addLock("username", "agent");
            controller.regenerateApiKey(1, "1234567890");
            fail();
        } catch (ApiException e) {
            assertEquals("Workspace 1 is locked", e.getMessage());
        }
    }
